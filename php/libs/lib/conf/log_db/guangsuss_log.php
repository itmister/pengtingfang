...
return [
    'guangsuss_log'     => [
        'software'             => 'guangsuss',
        'db_config_key'       => 'db_guangsuss_log',//数据配置config key
        'redis_key_prefix'    => 'logs:guangsuss:', //redis队列key前缀
...
CREATE TABLE `dosearch{[ymd]}` (
  ...
  `UUID` VARCHAR(255) DEFAULT NULL COMMENT 'Windows clinet生成的唯一健',
  `package_md5` VARCHAR(32) DEFAULT NULL COMMENT '安装包Md5',
  `search_type` VARCHAR(32) DEFAULT NULL COMMENT '搜索软件类型：local, baidu,sougou,360',
  `search_content` VARCHAR(255) DEFAULT NULL COMMENT '搜索 内容',
  PRIMARY KEY (`id`)
) ENGINE=MYISAM DEFAULT CHARSET=utf8 COMMENT='竞品数据';
            "
        ]
