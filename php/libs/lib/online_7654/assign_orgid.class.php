...
class Assign_orgid 
{
    ...
    public function __construct(){
        //初始化数据库模型
        $this->assignOrgidModel  = \Dao\Online_7654\Assign_Orgid::get_instance();
    }
    ...
    public function change_uid($vendor_data,$promotion,$ymd,$filepath,$fafang = 1)
    {
        ...
        //判断是否有交集数据
        if(!array_intersect($vendor_org_id_array,$o_arr) && $fafang == 2){
            //重新抓取邮件（邮件不完整）
            $vendor_where = "promotion = {$soft_id} AND ymd = {$ymd}";
            \Dao\Union\Vendor_Data_History::get_instance()->update($vendor_where, ['status' => 0]);
            return false;
        ...
        foreach($o_arr as $key => $org_id)
        {
            ...
            else
            {
                $temp = array(
                    'uid'           => $uid,            //用户id
                    'org_id'        => $org_id,         //软件渠道号或tn号
                    'soft_id'       => $soft_id,        //软件soft_id
                    'promotion_id'  => $promotion['id'],//软件id
                    'num'           => $num,            //有效量
