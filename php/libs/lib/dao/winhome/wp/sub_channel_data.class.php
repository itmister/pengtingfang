...
use \Dao;
class Sub_channel_data extends Wp {

    protected static $_instance = null;
    /**
     * @return Dao\Winhome\Wp\Sub_channel_data
     */
    ...
    public function in_sub_channel_data($ymd){
        $time =time();
        $sql = " insert into `{$this->_realTableName}` (ymd,qid,install,install_total,dateline)(SELECT $ymd,QID as qid,count(case when Ymd = {$ymd} then uid end) as `install`,count(*) as `install_total`,$time FROM wp_install_uid_qid_only GROUP BY QID) on duplicate key update install=values(install),install_total=values(install_total)";
        return $this->query($sql);
    }

    public function get_uninstall_count($ymd){
        $sql = "create table temp_qid_uninstall as
SELECT QID as qid,count(*) as `uninstall_total`,count(case when Ymd = {$ymd} then uid end) as `uninstall` FROM wp_uninstall_uid_qid_only GROUP BY QID;";
        $this->query($sql);
    ...
    public function get_online_count($ymd){
        $sql = "create table temp_qid_online as
                SELECT b.num,b.qid FROM `{$this->_realTableName}` as a LEFT JOIN (
                 SELECT count(*) as num , qid FROM wp_online_uid_qid_temp GROUP BY qid
                ) as b on a.qid=b.qid WHERE a.ymd={$ymd} and b.num is not NULL
        ";
        $this->query($sql);

        $up_sql = "UPDATE `{$this->_realTableName}` as a LEFT JOIN temp_qid_online as b on a.qid=b.qid set a.`online`=b.`num` WHERE a.ymd={$ymd};";
        $this->query($up_sql);
