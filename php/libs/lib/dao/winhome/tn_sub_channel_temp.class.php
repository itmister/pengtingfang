...
use \Dao;
class Tn_sub_channel_temp extends Winhome{

    protected static $_instance = null;
    /**
     * @return Dao\Winhome\Tn_sub_channel_temp
     */
    ...
    public function tn_update_is_me(){
        ...
        (SELECT sub_channel,soft_id,tn FROM wh_sub_channel_info where status=0 GROUP BY sub_channel,tn,soft_id) as tp on t.qid=tp.sub_channel and t.soft_id=tp.soft_id and t.tn=tp.tn
        set t.is_me=1 where tp.sub_channel is not NULL;";
    ...
    public function get_all_qid_data(){
        $sql = "SELECT ymd, qid,
                ...
                sum(case WHEN is_me=1 and soft_id='jsdh' THEN to_num else 0 end) as 'to_jsdh_num',
                sum(case WHEN is_me=1 and soft_id='jsdh' THEN to_ip_num else 0 end) as 'to_jsdh_ip_num',
