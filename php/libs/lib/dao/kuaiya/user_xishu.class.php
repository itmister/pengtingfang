...
class User_xishu extends  Kuaiya{
    ...
    public function get_xishu_list($qid = '',$limit = ''){
        $sql = "SELECT * FROM user_channel WHERE status = 1";
        if($qid)
        {
            if(strstr($qid,'_'))
            {
    ...
    public function user_xishu_data($cid_string,$start_date,$end_date)
    {
        ...
        $sql = "
            SELECT xishu,effective_date,(SELECT qid FROM user_channel WHERE id = cid) AS qid 
     ...
     */
    public function fetch_xishu($cid,$ymd){
        $sql = "SELECT xishu FROM {$this->_get_table_name()} WHERE cid = {$cid} AND effective_date <= {$start_date} ORDER BY modify_time DESC";
        $query = current($this->query($sql));
        return $query['xishu'] ? $query['xishu'] : 1;
    }
}
