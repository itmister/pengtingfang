...
use \Dao;
class Kunbang extends Huayangnianhua_log {
...
//        $sql = "show keys from `kunbang{$ymd}` WHERE key_name = 'uid'";
//        $index = $this->query($sql);
//        if(!$index){
//            $sql = "
//                    ALTER TABLE `kunbang{$ymd}`
...
//            $this->query($sql);
    ...
    private function _kunbang_temp_data($ymd){
        ...
        //捆绑数据
        $temp_table_name = "kunbang_temp{$ymd}";
        $sql = "
		...
		//分批添加
		$sql = "SELECT count(DISTINCT UID,kunbang_software) AS num FROM `kunbang{$ymd}` WHERE is_show = 1";
        ...
        for($i=1;$i<=$p_size;$i++){
			...
			$sql = "
				INSERT INTO `{$temp_table_name}` (UID,kunbang_software)
					SELECT 
						UID,kunbang_software FROM `kunbang{$ymd}` 
					WHERE 
						is_show = 1 
					GROUP BY 
    ...
    public function get_rate_date($ymd){
        ...
        //删除临时表
        $drop_sql = "DROP TABLE `kunbang_temp{$ymd}`;";
        $this->query($drop_sql);
