...
use \Dao;
class Install extends Huayangnianhua_log {

    /**
     * @return Dao\Huayangnianhua_log\Install
     */
    ...
    public function get_all_install_channel_qid($ymd,$limit=''){
        $sql = "
            SELECT UID as uid,substring_index(trim(QID),'_',1) as QID,{$ymd} as Ymd,repeat_install,Version as ver,OS AS os,is_wb FROM
    ...
    public function get_all_install_qid($ymd,$limit=''){
        $sql = "
            SELECT UID as uid,QID,{$ymd} as Ymd,repeat_install,Version as ver,is_wb FROM
            (SELECT * FROM `{$this->_realTableName}{$ymd}` order by `TimeStamp` ASC) 
            `{$this->_realTableName}{$ymd}` group by UID,substring_index(trim(QID),'_',1)
        ";
        if($limit){
