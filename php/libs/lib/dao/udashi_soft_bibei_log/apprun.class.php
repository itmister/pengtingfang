...
use \Dao;
class Apprun extends Udashi_soft_bibei_log {

    protected static $_instance = null;
    /**
     * @return Dao\Udashi_soft_bibei_log\Apprun
     */
     ...
     * @return array
     */
    public function get_bibei_count($ymd){
        $sql = "SELECT * from 
(SELECT count(DISTINCT UID) {$this->_realTableName}  from `{$this->_realTableName}{$ymd}` )a,
(SELECT count(DISTINCT UID) {$this->_realTableName}_zx  from `{$this->_realTableName}{$ymd}` where ONLINE=1)b,
(SELECT count(DISTINCT UID) {$this->_realTableName}_lx  from `{$this->_realTableName}{$ymd}` where ONLINE=0)c";
        return $this->query($sql)[0];
    }

    public function get_bibei_ver_count($ymd){
        $sql = "SELECT version as ver,{$ymd} as ymd,count(distinct UID) as online,UNIX_TIMESTAMP() as dateline from `{$this->_realTableName}{$ymd}` group by version";
    ...
    public function get_online_uuid_count($ymd){
        $sql = " SELECT count(*) as `online` from (SELECT UUID FROM `{$this->_realTableName}{$ymd}` group by UUID) as aa";
        return $this->query($sql);
    }
    ...
    public function get_all_online_uuid_count($ymd){
        $sql = "SELECT count(DISTINCT UUID) as num FROM `{$this->_realTableName}{$ymd}`";
        $data = $this->query($sql);
    ...
    public function get_all_online_channel_qid_uuid_count($ymd){
        $sql = "SELECT count(DISTINCT UUID,substring_index(QID, '_', 1)) as num FROM `{$this->_realTableName}{$ymd}`";
        $data = $this->query($sql);
        return isset($data[0]['num'])&&$data[0]['num']>0 ? $data[0]['num'] : 0;
    }
    ...
    public function get_all_online_qid_uuid_count($ymd){
        $sql = "SELECT count(DISTINCT UUID,QID) as num FROM `{$this->_realTableName}{$ymd}`";
        $data = $this->query($sql);
        return isset($data[0]['num'])&&$data[0]['num']>0 ? $data[0]['num'] : 0;
    }
    ...
    public function get_all_online_qid_uuid($ymd,$limit=''){
        $sql = "SELECT UUID as uid,QID as qid,{$ymd} as ymd FROM `{$this->_realTableName}{$ymd}` group by UUID,QID";
        if($limit){
            $sql .= " limit {$limit}";
        }
        return $this->query($sql);
