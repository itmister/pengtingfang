...
use \Dao;
class Tn_temp extends \Dao\Huayangnianhua_admin\Huayangnianhua_admin{
    ...
    public function tn_update_is_me(){
        ...
        (SELECT soft_id,tn FROM stat_sub_channel_info GROUP BY tn,soft_id) as tp on t.soft_id=tp.soft_id and t.tn=tp.tn
    ...
    public function get_tn_data($ymd){
        $sql = "SELECT {$ymd} as ymd,a.soft_id as soft_id,a.tn as tn,to_num,to_ip_num,UNIX_TIMESTAMP() as dateline
                FROM stat_sub_channel_info as a LEFT JOIN `{$this->_realTableName}` as b on a.soft_id=b.soft_id and a.tn=b.tn GROUP BY a.soft_id,a.tn";
        return $this->query($sql);
    }
