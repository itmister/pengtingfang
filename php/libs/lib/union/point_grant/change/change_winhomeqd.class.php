 ...
 * 原始渠道号转换uid
 */
class Change_winhomeqd extends Change{

    protected static $_instance = null;
    private static $_changeSoftId = "winhome";//取渠道号的softId
    private static $_getDataSoftId = "winhome";//取数据的softId

    /**
     * @return \Union\Point_grant\Change\Change_winhomeqd
     */
    ...
    public function org_id_change_my_org_id($array){
        ...
        $original_sum = 0;
        foreach($array as $_v){
            $arr_original[$_v['org_id']] = $_v['active_count'];
            if($_v['active_count']<=0){
                continue;
            }
            $arr[$_v['org_id']] = $_v['active_count'];
            $original_sum += $_v['active_count'];
        }
