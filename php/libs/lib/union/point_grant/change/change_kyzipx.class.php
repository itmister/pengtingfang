 ...
 * 原始渠道号转换uid
 */
class Change_kyzipx extends Change{

    protected static $_instance = null;
    private static $_changeSoftId = "";//取渠道号的softId
    private static $_getDataSoftId = "kyzip";//取数据的softId

    /**
     * @return \Union\Point_grant\Change\Change_kyzipx
     */
    ...
    public function get_data_to_vendor_org($softId,$ymd){
        ...
        $data = \Dao\Union\Vendor_Org_Data::get_instance()->select(
            array(
                "where"=>"promotion='{$softId}' and ymd={$ymd} and org_id like '7654_%'",
    ...
    public function org_id_change_my_org_id($array){
        ...
        $original_sum = 0;
        foreach($array as $_v){
            $_v['org_id'] = str_replace("7654_","",$_v['org_id']);
            $_v['org_id'] = intval($_v['org_id']);
            if($_v['org_id']<1000000) continue;
