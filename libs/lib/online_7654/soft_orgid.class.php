...
class Soft_orgid {
    ...
    public function get_list_by_soft_id( $soft_id ) {
    	...
    	$result = array();
    	$field = 'id,org_id';
    	$list   = \Dao\Online_7654\Soft_orgid::get_instance()->get_all($where,$field);
    ...
    public function assign_org_id($uid,$soft_id_array,$org_id_array){
    	...
    	foreach ($soft_id_array as $key => $val){
    		...
    		\Dao\Online_7654\Soft_orgid::get_instance()->update($where, $arr_data);

    		$map = "status = 0 and softID = '{$val}' and org_id in('{$str}')";
    		$assign_list = \Dao\Online_7654\Assign_orgid::get_instance()->get_all($map);
    		...
    		\Dao\Online_7654\Assign_orgid::get_instance()->addAll($data);
    	}
    	...
    	foreach ($un_assign_list as $uk => $uv){
    		...
    		\Dao\Online_7654\Soft_orgid::get_instance()->update($where, $un_data);
    		
    		$map = "softID = '{$uk}' and org_id in('{$ustr}') order by id asc";
    		$ao_list = \Dao\Online_7654\Assign_orgid::get_instance()->get_all($map);
    		foreach ($ao_list as $aok => $aov){
    			$d = array();
    			$d['status'] = $aov['status'] + 1;
    			if($aov['status'] == 0) $d['updateline'] = $un_data['hdateline'];
    			\Dao\Online_7654\Assign_orgid::get_instance()->update("id={$aov['id']}", $d);
    		}
    	}
    }
    ...
    public function write_log($where,$uid,$status){
    	...
    	\Dao\Online_7654\Soft_orgid_log::get_instance()->addAll($data);
    }
}