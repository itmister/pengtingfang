...
return [
    'udashi_soft_pro'     => [
        'software'             => 'udashipro',
        'db_config_key'       => 'db_udashi_soft_pro_log',//数据配置config key
        'redis_key_prefix'    => 'logs:udashipro:', //redis队列key前缀
...
CREATE TABLE `avaliable{[ymd]}` (
  ...
  `CpuID` VARCHAR(32) DEFAULT NULL COMMENT '软件标识',
  `QID` VARCHAR(32) DEFAULT NULL COMMENT '渠道ID',
  `guid` VARCHAR(255) DEFAULT NULL COMMENT 'U盘唯一标示',
  `Version` VARCHAR(255) DEFAULT NULL COMMENT '软件版本号',
  `kuaizip` INT(10) UNSIGNED DEFAULT NULL COMMENT '1包含快压，0不包含',
  PRIMARY KEY (`id`),
   KEY `UID` (`UID`)
) ENGINE=MYISAM DEFAULT CHARSET=utf8 COMMENT='系统日活接口数据';
            ",
            'updaterun' => "
...
CREATE TABLE `checknav{[ymd]}` (
  ...
  `QID` VARCHAR(32) DEFAULT NULL COMMENT '渠道ID',
  `Version` VARCHAR(255) DEFAULT NULL COMMENT '软件版本号',
  `flag` VARCHAR(255) DEFAULT NULL COMMENT '是否勾选【推荐导航】，1：勾选，0：不勾选',
  PRIMARY KEY (`id`)
) ENGINE=MYISAM DEFAULT CHARSET=utf8 COMMENT='安装接口数据';
            ",
        ]
