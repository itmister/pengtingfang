...
use \Dao;
class Online extends Mykzip_log {

    /**
     * @return Dao\Mykzip_log\Online
     */
    ...
    public function get_all_online_qid_count($ymd){
        $sql ="
            SELECT COUNT(a.QID) as num FROM(
                SELECT QID FROM `{$this->_realTableName}{$ymd}` GROUP BY UID,QID
            ) AS a
        ";
    ...
    public function get_all_online_ver_count($ymd){
        $sql ="
            SELECT COUNT(a.Version) as num FROM(
                SELECT Version FROM `{$this->_realTableName}{$ymd}` GROUP BY UID,Version
            ) AS a
        ";
        $data = current($this->query($sql));
        return $data['num'] ? $data['num'] : 0;
