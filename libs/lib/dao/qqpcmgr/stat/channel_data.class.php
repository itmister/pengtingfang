...
use \Dao\Qqpcmgr;
class Channel_data extends \Dao\Qqpcmgr\Qqpcmgr {
...
SELECT QID as qid,count(*) as `uninstall_total`,count(case when Ymd = {$ymd} then Ymd end) as `uninstall` FROM stat_uninstall_uid_channel_qid_only GROUP BY QID;";
    ...
    public function get_online_count($ymd){
        ...
        $up_sql = "UPDATE `{$this->_realTableName}` as a LEFT JOIN temp_channel_qid_online as b on a.qid=b.qid set a.`online`=b.`num` WHERE a.ymd={$ymd} and b.num>0;";
        $this->query($up_sql);
