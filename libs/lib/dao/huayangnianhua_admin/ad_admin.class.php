...
class Ad_admin extends  Huayangnianhua_admin{
    ...
    public function admin_list($where,$qid = '',$limit = ''){
        ...
        //获取渠道账号渠道号列表
        $sql = "
            SELECT '{$admin_info['username']}' AS username,id AS channel_id,qid FROM gs_user_channel
            WHERE status = 1 AND userid = {$admin_info['userid']}
        ";
        if($qid){
            $sql .= " AND qid LIKE '%{$qid}%'";
            
            if(strstr($qid,'_')){
                $sql .= " AND qid LIKE '%{$qid}%'";
            }else{
                $sql .= " AND qid LIKE '%{$qid}\_%'";
            }
        }
        if($limit){
            $sql .= " LIMIT {$limit}";
