...
use \Dao;
class Ver_data extends Wp {

    protected static $_instance = null;
    /**
     * @return Dao\Winhome\Wp\Ver_data
     */
    ...
    public function in_ver_data($ymd){
        $time =time();
        $sql = " insert into `{$this->_realTableName}` (ymd,ver,install_total,install,dateline)(SELECT $ymd,ver,count(*) as `install_total`,count(case when Ymd = {$ymd} then uid end) as `install`,$time FROM wp_install_uid_ver_only GROUP BY ver) on duplicate key update install=values(install),install_total=values(install_total)";
...
SELECT ver,count(*) as `uninstall_total`,count(case when Ymd = {$ymd} then uid end) as `uninstall` FROM wp_uninstall_uid_ver_only GROUP BY ver;";
        $this->query($sql);
...
create table temp_ver_online as
                SELECT b.num,b.ver FROM `{$this->_realTableName}` as a LEFT JOIN (
                 SELECT count(*) as num , ver FROM wp_online_uid_ver_temp GROUP BY ver
